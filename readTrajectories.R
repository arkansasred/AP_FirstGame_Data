...
readSubject<-function(subject){
    ...
    allLevels<-sapply(dirs_of_interest, list.files, full.names = TRUE)
    allTrajectories<-lapply(allLevels, readTrajectories)
    remove(allLevels)
    c<-read.table(str_join(subject,"/condition.txt"))
    colnames(c)<-"Condition"
    medianDevs<-list()
    for (i in 1:length(allTrajectories)){
        devs<-sapply(allTrajectories[[i]], getDev)
        medianDevs[[i]]<-devs
    }
    ...
    meanOfDevsByLevel<-sapply(medianDevs, mean)
...
readEverything<-function(){
    ...
    cond1<-apply(cond1Means, 1, mean)
    cond2<-apply(cond2Means, 2, mean)
    plot(1:length(cond1), cond1, pch=19, col="red", xlab = "Level", ylab="Mean of Median Abs. Deviations (pixels)", main = "Mean of Median Absolute Deviations from Enemies by Level")
